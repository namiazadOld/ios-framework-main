...
-(baseControl*) render: (NSMutableArray*) arguments container: (baseControl*)parent elements: (baseControl*) _elements
{
	...
	self.detail = [[detail alloc] init];
	self.master = [[master alloc] init];
...
-(void) initializeMaster: (customControl*) content
{
...
-(void) initializeDetail: (customControl*) content
{
...
-(void) manageArgument: (BindableObject*)bo at:(int)index
{
	...
	switch (index) {
		case 0:
		{
			[self initializeMaster:(customControl*)bo.value];
			break;
		}
		case 1:
		{
			[self initializeDetail:(customControl*)bo.value];
			break;
		}
		case 2:
		{
